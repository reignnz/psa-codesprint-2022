      ...
      <Stack sx={{width: '350px'}}>
        ...
        <form onSubmit={form.onSubmit((data) => submitForm(data))}>
          ...
          <Stack>
            ...
            <Group >
              <Stack onClick={() => {
                setSelectedRole(() => { return {STAFF: true, DO: false, AETOS: false, ADMIN: false}})
                form.setFieldValue("role", "STAFF")
              }} className="hover:cursor-pointer hover:bg-white rounded-md p-4" sx={{backgroundColor: selectedRole.STAFF ? 'white' : 'none'}}><HiOutlineUser className="mx-auto" size={20}/><Text>STAFF</Text></Stack>
              <Stack onClick={() => {
                setSelectedRole(() => { return {STAFF: false, DO: true, AETOS: false, ADMIN: false}})
                form.setFieldValue("role", "DESIGNATED_OFFICER")
              }} className="hover:cursor-pointer hover:bg-white rounded-md p-4" sx={{backgroundColor: selectedRole.DO ? 'white' : 'none'}}><HiOutlineUsers className="mx-auto" size={20}/><Text>D.O.</Text></Stack>
              <Stack onClick={() => {
                setSelectedRole(() => { return {STAFF: false, DO: false, AETOS: true, ADMIN: false}})
                form.setFieldValue("role", "AETOS")
              }} className="hover:cursor-pointer hover:bg-white rounded-md p-4" sx={{backgroundColor: selectedRole.AETOS ? 'white' : 'none'}}><RiShieldCrossLine className="mx-auto" size={20}/><Text>AETOS</Text></Stack>
            </Group>
          </Stack>
